...
- **친밀도 + 경험치** — 함께한 시간이 쌓이면 레벨업 + 친밀도 상승, 진화도 자동
- **야생 인카운터** — 화면에 야생 포켓몬이 나타나면 클릭해서 잡기 (희소한 레어 변종 1%)
- **아이템 시스템** — 가방에 떨어진 사과/장난감/몬스터볼 수집, 진화석 사용
- **도감 + 가방** — Gen 1 (#0001~#0151) + 사용자가 만든 커스텀까지 일관된 UI로 관리
- **커스텀 포켓몬 등록** — 직접 그린 GIF로 나만의 종 추가 (자동으로 야생 풀에도 합류)
- **일일 인사 + 알람** — 출근/점심/퇴근 인사, 사용자 정의 리마인더
- **다양한 스프라이트 스타일** — BW 도트 / 쇼다운 애니메이션 토글
...
| 이름 변경 | 닉네임 설정/해제 (히스토리 자동 저장) |
| 내 포켓몬 | 가방 + 파티 관리 |
| 내 가방 | 인벤토리 — 진화석·물약·마스터볼 사용 |
| 도감 | 잡은 포켓몬 컬렉션 |
| 리마인더 설정 / 일일 일정 | 알람 관리 |
| 야생 포켓몬 소환 (테스트) | 인카운터 강제 발동 |
| 커스텀 포켓몬 추가 | GIF로 새 종 등록 |

## 커스텀 포켓몬 추가

1. **GIF 준비** — 권장 128×128, 24~28 frames, 1MB 이하. 큰 GIF는 함께 제공되는 `pokemon-maker/optimize_pokemon_gifs.py`로 자동 최적화 가능
2. 트레이 → **"커스텀 포켓몬 추가…"**
