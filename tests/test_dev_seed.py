...
from pokemon_buddy.dev_seed import ITEM_MAX
from pokemon_buddy.items import ITEMS, ItemKind


def test_seed_maxes_every_item(store, temp_assets):
    dev_seed.seed_test_data(store)
    for it in ITEMS:
        assert store.get_item_count(it.key) >= ITEM_MAX
    # skill scroll is an item too
...
def test_seed_is_repeatable(store, temp_assets):
    ...
    dev_seed.seed_test_data(store)
    assert store.get_item_count("food.apple") >= ITEM_MAX
